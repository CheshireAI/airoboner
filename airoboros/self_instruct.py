...
async def example_task(name):
    try:
        ...
        if random.choice([True, False]):
            raise ValueError(f"Random error in task {name}")
        logger.debug(f"Task {name} completed")
    except Exception as e:
        logger.error(f"Task {name} encountered an error: {e}")

async def main():
    tasks = [asyncio.create_task(example_task(f"Task-{i}")) for i in range(5)]
    await asyncio.gather(*tasks)

if __name__ == "__main__":
    logger.debug("Starting main program")
    asyncio.run(main())
    logger.debug("Main program completed")
...
OPENAI_API_BASE_URL = "http://127.0.0.0:7860"
